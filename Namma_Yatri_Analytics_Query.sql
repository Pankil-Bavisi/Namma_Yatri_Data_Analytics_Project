...
-- Table: Trips_Details4 (Main Table)
...
-- Q1. Total numbers of Trips

select count(distinct tripid) from trips_details

-- Checking Duplicate Tripid if it is present there! group by based on Tripid and count if Tripid is > 1, means there are 
-- Duplicate Tripid present there.
...
-- Q2. Total numbers of Drivers

select count(distinct driverid) total_drivers from trips


-- Q3. Total Earnings from the Trips

select sum(fare) fare from trips


-- Q4. Total number of Completed Trips (In Trips TB, these are Successfully completed trips)

select count(distinct tripid) trips from trips


-- Q5. Total number of Searches
...
-- Q7. Total number of Trips Cancelled by the Driver 

select count(*) - sum(driver_not_cancelled) driver_not_cancelled from trips_details


-- Q8. Total number of OTP entered
 
 select sum(otp_entered) otp_entered from trips_details


-- Q9. Total number of End Ride

select sum(end_ride) total_end_ride from trips_details


-- Q10. Average Distance per Trips

select avg(distance) avg_distance from trips


-- Q11. Average Distance per Trips

select avg(fare) avg_fare from trips


-- Q12. Total number of Distance Travelled

select sum(distance) tot_distance from trips


-- Q13. Which is the most use Payment Method?
...
-- Q14. Highest payment was made through which instrument?

--1'st Way: For particular Trip
select a.method from payment a inner join
(select top 1 * from trips
order by fare desc) b
on a.id=b.faremethod

--2'nd Way: For all Payment method

select a.method from payment a inner join
(select top 1 faremethod, sum(fare) tot_fare from trips
group by faremethod
order by sum(fare) desc) b
on a.id=b.faremethod


-- Q15. Which 2 Locations had the highest trips
...
-- Q18. Which Driver, Customer pair had more orders?
...
-- Q20. Estimate to serach for quote rate

select sum(searches_for_quotes)*100.0/sum(searches_got_estimate) from trips_details

-- Q21. search for quote to driver acceptance rate

select sum(searches_for_quotes)*100.0/sum(searches_got_estimate) from trips_details

-- Q22. quote to booking rate

select sum(searches_got_quotes)*100.0/sum(searches_for_quotes) from trips_details


-- Q23. Booking cancellation rate

select sum(driver_not_cancelled)*100.0/sum(searches_got_quotes) as cancellation_rate from trips_details
-- select 100 - sum(driver_not_cancelled)*100.0/sum(searches_got_quotes) as cancellation_rate from trips_details
...
-- Q25. Which Area got highest trips in which Duration

--Pro:1 - Which duration got the highest number of Trips, in each of the location/area  present
...
--Pro:2 - Which location/area got the highest number of Trips, in which duration
...
-- Q26. Which area got the highest fares, cancellation, trips?
...
(select loc_from, count(*) - sum(driver_not_cancelled) cancellation from trips_details
group by loc_from) a) b
where rnk=1
...
select * from
(select *, rank() over(order by cancellation desc) rnk from
(select loc_from, count(*) - sum(customer_not_cancelled) cancellation from trips_details
group by loc_from) a) b
where rnk=1

-- Highest Trips [trips = 35-39-1 | trips_details = 1-75-1]
select * from
(select *, rank() over (order by tot_trips desc) rnk from
(select loc_from, count(distinct tripid) tot_trips from trips_details
group by loc_from) a) b
where rnk=1


-- Q27. Which duration got the highest fares and trips
...
select * from
(select *, rank() over (order by tot_trips desc) rnk from
(select duration, count(distinct tripid) tot_trips from trips
group by duration) a) b
where rnk=1